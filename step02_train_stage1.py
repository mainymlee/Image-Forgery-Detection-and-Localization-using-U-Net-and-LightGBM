"""
Step 02 v2 — Stage 1 U-Net++ 학습 (1-run 개선 실험)

흐름:
  train 데이터를 group_id 기준으로 1번 무작위 분할 (내부 train 80% / 내부 val 20%)
  → 각 분할로 U-Net++ 학습 (max 50 epoch, early stopping patience=10)
  → 1개 best checkpoint를 최종 Stage 1 모델로 선택
...
sys.path.insert(0, "/content/drive/MyDrive/forensic_project_v2")
...
CFG = {
    ...
    "pos_weight"    : 4.0,      # v2: 조작 픽셀 가중치 강화
    "n_repeats"     : 1,        # v2: 1-run 개선 실험
    "seeds"         : [42],
    "inner_val_ratio": 0.20,    # train 내부 val 비율
    "num_workers"   : 2,        # OOM/worker 오류 시 0으로 낮추기
}
...
if __name__ == "__main__":
    ...
    for i, seed in enumerate(CFG["seeds"]):
        print(f"\n{'='*60}")
        print(f"🔁 v2 Hold-out  Run {i+1}/{CFG['n_repeats']}  (seed={seed})")
        print(f"{'='*60}")
