...
def load_image_letterbox(path, target=512):
...
def load_mask_letterbox(path, target=512):
...
for rank, idx in enumerate(targets):
    ...
    img = load_image_letterbox(row["image_path"], 512)
    heatmap = load_heatmap(row["heatmap_path"])
    gt_np = load_mask_letterbox(row["mask_path"], 512)
